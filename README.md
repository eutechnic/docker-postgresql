...
Automated builds of the image are available on [Dockerhub](https://hub.docker.com/r/eutechnic/postgresql) and is the recommended method of installation.

```bash
docker pull eutechnic/postgresql:10.3
```

Alternatively you can build the image yourself.

```bash
docker build -t eutechnic/postgresql github.com/eutechnic/docker-postgresql
```
...
```bash
docker run --name postgresql -itd --restart always \
  eutechnic/postgresql:10.3 -c log_connections=on
```
...
```bash
docker run --name postgresql -itd --restart always \
  eutechnic/postgresql:10.3 -c logging_collector=on
```
